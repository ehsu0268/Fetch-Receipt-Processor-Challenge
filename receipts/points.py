...
class PointsCalculator:
    ...
    def determine_points(receipt):
        print(
            "Retail: "
            + str(RetailProcessor.calculate(receipt["retailer"]))
            + "\n"
            + "Total: "
            + str(TotalProcessor.calculate(receipt["total"]))
            + "\n"
            + "Items: "
            + str(ItemsProcessor.calculate(receipt["items"]))
            + "\n"
            + "Day: "
            + str(DayProcessor.calculate(receipt["purchaseDate"]))
            + "\n"
            + "Time: "
            + str(TimeProcessor.calculate(receipt["purchaseTime"]))
        )
        return (
            RetailProcessor.calculate(receipt["retailer"])
            + TotalProcessor.calculate(receipt["total"])
            + ItemsProcessor.calculate(receipt["items"])
            + DayProcessor.calculate(receipt["purchaseDate"])
...
class TotalProcessor:
    ...
    @staticmethod
    def calculate(input: str):
        return TotalProcessor.round_dollar_amount(
            input
        ) + TotalProcessor.multiple_of_quarter(input)
...
class ItemsProcessor:
    ...
    @staticmethod
    def calculate(items):
        return ItemsProcessor.number_of_items(items) + ItemsProcessor.item_description(
            items
        )
